...
class Image
{
    ...
    public function resize($filename, $max_size = 700)
    {
        ...
        // Check quel type de fichier c'est
        ...
        if (file_exists($filename)) {
            ...
            // Une fois l'image chargée, on peut la redimensionner
            $src_w = imagesx($image);
            $src_h = imagesy($image);

            if ($src_w > $src_h) {
                // Réduit la taille max si l'image est plus petite si elle est plus large
                if ($src_w < $max_size) {
                    $max_size = $src_w;
                }
                ...
                // Réduit la taille max si l'image est plus petite si elle est plus haute
                if ($src_h < $max_size) {
                    $max_size = $src_h;
                }
            ...
            if ($type == 'image/png') {
                imagealphablending($dst_image, false); // Permet de garder la transparence
    ...
    // Fonction qui permet de rogner une image
